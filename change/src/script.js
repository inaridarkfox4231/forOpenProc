...
class counter{
  constructor(){
    ...
    // diff廃止（色々いじります）
    // 今気付いたけどisOnって要らないのか？actorのisActiveを消せばいいのでは
  }
  ...
  step(diff = 1){
    this.cnt += diff; // カウンターの・・あ、これマイナスでもいいんだ・・一般のカウンターは。目からウロコ。
    // そうだよ。
    //if(this.cnt > lim){ this.cnt = lim; }
  } // よく考えたらonとかoffとか・・だって使う側がlimit超えましたねって勝手に判断すれば済む話じゃんな。
  //pause(){ this.isOn = !this.isOn; } // ポーズ要るかどうか
}

// limitedCounter要らないので廃止。
// limit超えたら勝手に処理止めちゃえばいいんだから。
...
class flow{
  ...
  initialize(_actor){}
  // 今気付いたけどactivateしてないとここは実行されないしstate変えるのはやっぱactor自身でしょ。
  ...
  */
  convert(_actor){
    //_actor.setState(IDLE); // IDLEに戻す って思ったけどやっぱIDLEにするのはactor自身なのでここは書かないかな・・
...
class assembleHub extends flow{
  ...
  // よく考えたら何もしないってありえないわ。
  execute(_actor){
    if(this.open){ _actor.setState(COMPLETED); } // 開いてるなら行って良し
    this.volume--; // 出て行ったら減らす
    if(this.volume === 0){ this.open = false; } // 0になったタイミングで閉じる
  }
  // やったできた。簡単じゃないかー。
  // convertFuncでvolumeと連携させれば「散開」も可能になるはず（ばーーってやつ）→別のhubでやってくれ・・・・
  // ていうか処理を分ける、ね。基本がなってないんだよな。
  /*
  convert(_actor){
    let nextFlow = all.getNextFlow(this.index, this.convertFunc(this, _actor));
    _actor.state = nextFlow;
...
class jumpFlow extends orbitalFlow{
  ...
  execute(_actor){
    // actorを止めるにはinActivate使えばいいのでgetStateうんぬんはカット
    /*_actor.timer.step(_actor.speed);　// 進めて・・
    let cnt = _actor.timer.getCnt();  // カウントゲットして・・
    if(cnt >= this.span){
      cnt = this.span;
      _actor.setState(COMPLETED); // これでいい。だから普通のcounterでいいのよね
    }*/
    ...
    //_actor.pos.y -= (2 / this.span) * cnt * (this.span - cnt);
    _actor.pos_y -= 2 * this.span * progress * (1 - progress); // 高さはとりあえずthis.span/2にしてみる
  }
}
...
class shootingFlow extends flow{
  ...
  initialize(_actor){
    //this.initialFunc(this, _actor)
    //_actor.pos.set(this.from.x, this.from.y); // 着いたときの位置情報を使うのでこれは不要
    // 着いたときに位置情報が与えられている時しか使わなければいいだけの話でしょ？
...
class fallFlow extends shootingFlow{
  constructor(vx, vy, gravity){
    super(); // fromは廃止
    this.vx = vx; // と思ったけど初速度用意しよ。向きとか設定できなくなる。
    this.vy = vy; // 加速度vx, 初期速度vy. vyは正の数にしないと落ちない
...
class throwFlow extends shootingFlow{
  ...
  execute(_actor){
    //if(!_actor.timer.getState()){ return; }
    //_actor.timer.step();
    //let cnt = _actor.timer.getCnt(); // カウンター使ってないじゃん。あほか。
...
class entity{
  constructor(){
    ...
    // this.convertList = []; // convertListは廃止。
    // updateされるflowってのを作りたいわね。一定時間ごとにgenerateするHubとか作りたいよね。
    this.actors = [];
